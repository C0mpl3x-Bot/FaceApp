...
def Output(self,win):
    ...
    try:
        #lets user select a file on their device.
        ...
        pop.title("Admin Upload Image Results") 
        ...
        #converts name to lowercase then checks if the extensions are either png/jpg
        if(name.lower()  == "png" or name.lower() == "jpg"):
            #set message to new output.(Successful output)
            message = "The file you have selected has been moved into our Admin system"
            #moves the file that the user selected to the img/known folder
            shutil.move(filename, './img/knownAdmin')
...
def close_pop():
    ...
    if messagebox.askokcancel("Quit", "Do you want to quit Admin Upload Images?"):
        pop.destroy() 